...
const AnalyticsPage = {
  ...
  bucketData() {
    ...
    const allowances = DB.data.allowances.filter((a) => Utils.inRange(a.date, range));
    ...
    if (range === 'daily') {
      ...
      keyFn = (date) => new Date(`${date}T00:00:00`).toLocaleDateString(undefined, { weekday: 'short' });
    } else if (range === 'monthly') {
      const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
      labels = Array.from({ length: daysInMonth }, (_, i) => `${i + 1}`);
      keyFn = (date) => String(new Date(`${date}T00:00:00`).getDate());
    } else {
      labels = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
      keyFn = (date) => labels[new Date(`${date}T00:00:00`).getMonth()];
    }
  ...
  render() {
    ...
    const dayCount = { daily: 1, weekly: 7, monthly: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate(), yearly: 365 }[range];
