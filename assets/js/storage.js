...
class StorageService {
  ...
  // ---------------- Allowances ----------------
  addAllowance({ amount, source, notes, date, time }) {
    const entry = {
      id: this.genId('al'),
    ...
    this.data.balance = Round2(this.data.balance + entry.amount);
  ...
  updateAllowance(id, updates) {
    ...
    this.data.allowances[idx] = { ...old, ...updates, amount: Number(updates.amount) };
    this.data.balance = Round2(this.data.balance + diff);
  ...
  deleteAllowance(id) {
    ...
    this.data.balance = Round2(this.data.balance - removed.amount);
  ...
  deletePlant(id) {
    ...
    // Return unwatered savings back to balance
    this.data.balance = Round2(this.data.balance + removed.saved);
  ...
  waterPlant(id, amount, notes) {
    ...
    this.data.balance = Round2(this.data.balance - amount);
    ...
    this.data.plants.forEach((p) => {
      p.history.forEach((h) => {
        waterings.push({
          ...
          createdAt: new Date(`${h.date}T${h.time || '00:00'}`).getTime() || 0,
          plantId: p.id
        });
      });
    });
